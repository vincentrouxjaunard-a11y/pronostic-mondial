...
// Couleurs cycliques pour les joueurs
...
const GROUPS = [
  ...
  { id:"E", name:"Groupe E", teams:["Allemagne 🇩🇪","Curaçao 🇨🇼","Côte d'Ivoire 🇨🇮","Équateur 🇪🇨"], matches:[
    ...
    {id:"E3",home:"Allemagne 🇩🇪",away:"Côte d'Ivoire 🇨🇮",date:"21 juin"},
  ...
  { id:"F", name:"Groupe F", teams:["Pays-Bas 🇳🇱","Japon 🇯🇵","Suède 🇸🇪","Tunisie 🇹🇳"], matches:[
    ...
    {id:"F3",home:"Pays-Bas 🇳🇱",away:"Suède 🇸🇪",date:"21 juin"},
    {id:"F4",home:"Japon 🇯🇵",away:"Tunisie 🇹🇳",date:"21 juin"},
  ...
  { id:"G", name:"Groupe G", teams:["Belgique 🇧🇪","Égypte 🇪🇬","Iran 🇮🇷","Nouvelle-Zélande 🇳🇿"], matches:[
    ...
    {id:"G3",home:"Belgique 🇧🇪",away:"Iran 🇮🇷",date:"22 juin"},
    {id:"G4",home:"Égypte 🇪🇬",away:"Nouvelle-Zélande 🇳🇿",date:"22 juin"},
  ...
  { id:"H", name:"Groupe H", teams:["Espagne 🇪🇸","Cap-Vert 🇨🇻","Arabie saoudite 🇸🇦","Uruguay 🇺🇾"], matches:[
    ...
    {id:"H3",home:"Espagne 🇪🇸",away:"Arabie saoudite 🇸🇦",date:"22 juin"},
    {id:"H4",home:"Cap-Vert 🇨🇻",away:"Uruguay 🇺🇾",date:"22 juin"},
  ...
  { id:"I", name:"Groupe I", teams:["France 🇫🇷","Sénégal 🇸🇳","Irak 🇮🇶","Norvège 🇳🇴"], matches:[
    ...
    {id:"I3",home:"France 🇫🇷",away:"Irak 🇮🇶",date:"23 juin"},
    {id:"I4",home:"Sénégal 🇸🇳",away:"Norvège 🇳🇴",date:"23 juin"},
    {id:"I5",home:"France 🇫🇷",away:"Norvège 🇳🇴",date:"27 juin"},
    {id:"I6",home:"Sénégal 🇸🇳",away:"Irak 🇮🇶",date:"27 juin"},
  ]},
  { id:"J", name:"Groupe J", teams:["Argentine 🇦🇷","Algérie 🇩🇿","Autriche 🇦🇹","Jordanie 🇯🇴"], matches:[
    ...
    {id:"J3",home:"Argentine 🇦🇷",away:"Autriche 🇦🇹",date:"23 juin"},
    {id:"J4",home:"Algérie 🇩🇿",away:"Jordanie 🇯🇴",date:"23 juin"},
  ...
  { id:"K", name:"Groupe K", teams:["Portugal 🇵🇹","RD Congo 🇨🇩","Ouzbékistan 🇺🇿","Colombie 🇨🇴"], matches:[
    ...
    {id:"K3",home:"Portugal 🇵🇹",away:"Ouzbékistan 🇺🇿",date:"24 juin"},
    {id:"K4",home:"RD Congo 🇨🇩",away:"Colombie 🇨🇴",date:"24 juin"},
  ...
  { id:"L", name:"Groupe L", teams:["Angleterre 🏴󠁧󠁢󠁥󠁮󠁧󠁿","Ghana 🇬🇭","Croatie 🇭🇷","Panama 🇵🇦"], matches:[
    ...
    {id:"L2",home:"Croatie 🇭🇷",away:"Panama 🇵🇦",date:"18 juin"},
    {id:"L3",home:"Angleterre 🏴󠁧󠁢󠁥󠁮󠁧󠁿",away:"Croatie 🇭🇷",date:"24 juin"},
    {id:"L4",home:"Ghana 🇬🇭",away:"Panama 🇵🇦",date:"24 juin"},
    {id:"L5",home:"Angleterre 🏴󠁧󠁢󠁥󠁮󠁧󠁿",away:"Panama 🇵🇦",date:"28 juin"},
    {id:"L6",home:"Ghana 🇬🇭",away:"Croatie 🇭🇷",date:"28 juin"},
  ]},
];
...
const computeQualPointsDetailed = (playerQuals, officialQuals) => {
  ...
  return { pts: team1Found + team2Found + perfectOrder, breakdown: { team1: team1Found, team2: team2Found, bonus: perfectOrder } };
};
...
const loadState = () => {
  try {
    ...
    return { ...fresh, predictions: saved.predictions ?? fresh.predictions, quals: saved.quals ?? fresh.quals, actual: { ...fresh.actual, ...(saved.actual ?? {}) }, officialQuals: saved.officialQuals ?? fresh.officialQuals, tab: saved.tab ?? "prono", activePlayer: saved.activePlayer ?? PLAYERS[0], activeGroup: saved.activeGroup ?? "A" };
  } catch { return buildFreshState(); }
};

const saveState = (state) => {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ predictions: state.predictions, quals: state.quals, actual: state.actual, officialQuals: state.officialQuals, tab: state.tab, activePlayer: state.activePlayer, activeGroup: state.activeGroup })); }
...
const C = {
  bg: "#080f1e", card: "#132038", border: "#1e3a5f",
...
const ScoreInput = ({ value, onChange, disabled }) => (
  <input
    ...
    inputMode="numeric"
    style={{
      width: 52, height: 52, textAlign: "center", fontSize: 22, fontWeight: 700,
      border: disabled ? "2px solid #334155" : "2px solid #22d3ee",
      borderRadius: 10, background: disabled ? "#1e293b" : "#0f172a",
      color: disabled ? "#64748b" : "#e2e8f0", outline: "none",
      cursor: disabled ? "not-allowed" : "text", fontFamily: "monospace",
...
export default function App() {
  ...
  const GroupSelect = () => (
    <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
      {GROUPS.map(g => (
        <button key={g.id} style={S.groupBtn(activeGroup===g.id)} onClick={() => dispatch({ type:"SET_GROUP", group:g.id })}>
          {g.id}
        </button>
      ))}
    </div>
        ...
        {curGroup && (
          <div style={S.card}>
            ...
            {curGroup.matches.map(m => {
              ...
              // Raccourci nom équipe pour mobile
    ...
    return (
      <div>
        <div style={{ ...S.card, background:"#1a0f2e", borderColor:"#4c1d95", marginBottom:14 }}>
          <div style={{ fontSize:13, color:C.purple, fontWeight:600 }}>✏️ Mode admin — Entrez les scores et qualifiés officiels</div>
        </div>
        <GroupSelect />
        {curGroup && (
          <div style={S.card}>
                ...
                <div key={m.id} style={{ marginBottom:14 }}>
                  <div style={{ fontSize:11, color:C.muted, marginBottom:6 }}>
                    ...
                    {act&&act.home!==undefined&&!isNaN(act.home)
                      ?<span style={{color:C.green,marginLeft:8}}>→ {act.home}-{act.away}</span>
                      :<span style={{color:C.muted,marginLeft:8}}>Pas encore joué</span>}
                  </div>
      ...
      <div style={{ marginTop:16 }}>
        {!resetConfirm
          ...
          : <div style={{ ...S.card, borderColor:C.red, background:"#1a0a0a" }}>
              <div style={{ fontSize:13, color:C.red, marginBottom:10, fontWeight:600 }}>Confirmer ? Tous les pronostics seront effacés.</div>
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"'Inter',-apple-system,sans-serif", paddingBottom:60 }}>
      ...
      <div style={S.header}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          ...
          <div style={{ fontSize:11, color:saved?C.green:"transparent", transition:"color 0.3s", paddingTop:4 }}>✓ Sauvegardé</div>
        </div>

        {/* Mini classement scrollable horizontal */}
